...
class SchemaPiece extends Schema {
	...
	constructor(client, gateway, options, parent, key) {
		...
		/**
		  * Whether or not this key should hold resolved instances of this.type or not.
			* @since 0.5.0
			* @type {boolean}
			*/
	 ...
	 * @returns {*}
	 */
	async parse(value, guild, { saving, silent = false }) {
		const resolved = await this.gateway.resolver[this.type](value, guild, this.key, { min: this.min, max: this.max, resolve: saving !== undefined ? saving : this.resolve, def: this.default }, silent);
	...
	/**
	 * Checks if options.resolve is valid.
	 * @since 0.5.0
	 * @param {boolean} resolve The parameter to validate
	 * @private
	 */
	 _schemaCheckResolve(resolve) {
		 if (typeof resolve !== 'boolean') throw new TypeError(`[KEY] ${this} - Parameter resolve must be a boolean.`);
	 }
	...
	_init(options) {
		...
		this._schemaCheckResolve(this.resolve);
